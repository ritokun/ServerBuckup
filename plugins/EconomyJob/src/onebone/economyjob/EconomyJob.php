...
class EconomyJob extends PluginBase implements Listener{
	...
	private function readResource($res){
		...
		if(!is_resource($resource)){
			$this->getLogger()->debug("未知のリソースを読み込もうとしました ".TextFormat::AQUA.$res.TextFormat::RESET);
			return false;
		}
		...
		switch(array_shift($params)){
			case "join":
				if(!$sender instanceof Player){
					$sender->sendMessage("このコマンドはゲームで実行してください。");
				}
				if($this->player->exists($sender->getName())){
					$sender->sendMessage("あなたはもう仕事に参加しています。");
				}else{
					...
					if($this->jobs->exists($job)){
						...
						$sender->sendMessage("職業に入りました \"$job\"");
					}else{
						$sender->sendMessage("今のJOBの名前がありません \"$job\"");
					}
				}
				break;
			case "retire":
				...
				if($this->player->exists($sender->getName())){
					...
					$sender->sendMessage("職業から抜けました \"$job\"");
				}else{
					$sender->sendMessage("まだ職業に参加していません");
				}
				break;
			case "list":
				...
				$output = "求人一覧ページを表示する $page of $max : \n";
			...
			case "me":
				...
				if($this->player->exists($sender->getName())){
					$sender->sendMessage("あなたの職業 : ".$this->player->get($sender->getName()));
				}else{
					$sender->sendMessage("まだ職業に参加していません");
				}
				break;
